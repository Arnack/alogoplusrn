...
@router.message(Manager(), F.text == '✍️ Уведомление')
...
@router.callback_query(Manager(), F.data == 'Newsletter')
...
@router.callback_query(Manager(), F.data.startswith('NewsletterCity:'))
...
@router.message(Manager(), StateFilter("NewsletterMessage"))
...
@router.callback_query(Manager(), F.data == 'NewsletterCancel')
async def newsletter_cancel(
        callback: CallbackQuery
):
